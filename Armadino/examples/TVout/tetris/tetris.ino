...
#ifdef ARMADINO
#define STARTMSG "Press A to start"
#include <Armadino.h>
Armadino rmdn;
LedMatrix m;
...
void setup() {
...
#ifdef ARMADINO
  rmdn.begin(); TV.begin(PAL,W,H); TV.set_hbi_hook(Armadino::ledRefresh);
  m.loadBitmap(icon);
  rmdn.show(m);
#else
  TV.begin(PAL,W,H);
  ...
  TV.select_font(font6x8);
  TV.delay(500);
  TV.printPGM(10,60,PSTR(STARTMSG));
#ifdef HACKVISION
  TV.printPGM(0,H-16,PSTR("To pause, press"));
  TV.printPGM(0,H-8,PSTR("< & > together"));
#endif
#ifdef ARMADINO
  TV.printPGM(5,H-16,PSTR("To pause, press B"));
